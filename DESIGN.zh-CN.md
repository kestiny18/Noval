...
- **子进程类工具**（run_bash、外部命令）：统一走 `ProcessRuntime.run(timeout=N)`，超时能终止直接启动的子进程；后代进程树是否完整清理必须看 backend capability，不扩大承诺。✅
...
- **去掉悬空的 `Tool.timeout` 字段**：它定义了但 executor 从不消费，会误导人以为"框架统一超时"。
  run_bash 的 timeout 仍来自工具参数，现在由 `ProcessRuntime.run()` 实施；纯函数无法安全强杀（决策 4）。
  框架级 timeout 字段属"假承诺"，删掉——超时是工具的属性，不是框架的。
...
- **对话轮次**（canonical user / assistant / tool blocks）是**持久态**：**一条都不能少**。
...
### 18.4 信封格式：`{seq, ts, message}`，时间不进消息体

每条消息的落盘时刻必须存（排序、UI 显示「3 天前」、延迟分析），但不能塞进 canonical message，否则会跟着请求进入 Provider。所以每行使用框架信封，消息体从 v0.9 起只保存 canonical schema：

```json
{"seq":7,"ts":"2026-06-23T15:30:12.123+08:00","message":{"role":"user","blocks":[{"type":"text","text":"<context>...</context>\n\nhello"}]}}
```

- 加载即剥信封并解析 `ConversationMessage`；`ts`/`seq` 留在框架层，Provider 永远看不到。
- `ts` 用**带时区 ISO8601**（`now().astimezone()`），跨时区/DST 不歧义。
- `seq` 单调序号，为未来压缩（`summary_of:[3,4,5]`）、DB 主键、半截写去重铺路，近乎免费。
- 首行同形信封带 `_meta`：`{"_meta":{"schema_version":2,"session_id":...,"created_at":...,"workdir":...,"model":...}}`。v1 文件保留但不迁移、不读取。
...
DeepSeek 工具轮的 `reasoning_content` 与 Anthropic thinking / redacted-thinking blocks 都可能是继续工具链所必需的协议状态，但它们不是跨 Provider 语义。v0.9 起适配器把这类状态放入带 adapter id 与 schema version 的 opaque replay state；禁止退回 `model_dump()` 把 annotations/audio 等无关字段一并灌进历史。

Agent 与 SessionStore 都不解释 replay payload，只原样编排和持久化；只有所属 adapter 能在发请求时读取它，foreign adapter、compactor、judge 和日志使用不含 replay/provenance 的 semantic view。原始思考不展示；`LLMResponse.meta` 只放 `thinking_enabled / duration_ms` 等框架指标，Provider 返回的 prompt / completion / cache / reasoning token 统一进入 `TokenUsage`。Agent 汇总一个用户回合内的 reasoning 与耗时，CLI 展示 token、耗时和工具调用数。
...
原始 `<session>.jsonl` 继续逐条保存 canonical user / assistant / tool 消息，是唯一真相源。压缩器只读取带 `seq/ts/message` 的 `SessionRecord` semantic view，在完整最终 assistant 回复处选择边界，并把结构化摘要追加到 `context/<session>.jsonl`。checkpoint 记录来源 seq 区间、上一个 checkpoint、来源哈希、时间、模型和 prompt version；摘要不写回原始消息，也不提升为 system 权限。
...
配置来源分两层：用户级 `~/.noval/mcp.json` 与项目级 `<workdir>/.noval/mcp.json`。项目级配置收进 `.noval/`，避免在仓库根目录散落 Noval 专属文件，同时与用户级文件名保持一致。启动时只把 server 的轻量索引注入 system prompt，包含 id/name/source/transport/env key，不暴露 env 值，也不自动启动外部进程。模型需要具体能力时，必须先用 `list_mcp_tools(server=...)` 按需连接 server 读取工具列表，再用 `call_mcp_tool(...)` 执行。这样 system 前缀保持轻，外部进程启动也不会在用户未触发时发生。
...
path-jail 不是 OS 沙箱，而是给进程内 Python 文件工具补上的逻辑边界。第一版只管 `read_file` / `list_directory` / `glob` / `grep` / `write_file` / `edit_file`，统一从 `builtins._resolve` 进入 `ConfinementPolicy` 判定；`run_bash`、Skill 脚本和 MCP 外部进程不会因此变安全，它们走独立的 `ProcessRuntime` 与平台沙箱接缝。
...
## 决策 30：统一子进程运行时先于平台硬沙箱

`run_bash`、Skill script、环境探测和 MCP 都会启动外部进程，但生命周期并不相同：前几类是一次性命令，MCP 是由官方 SDK 持有的长驻双向 stdio transport。因此不能把所有入口粗暴塞进一个 `confined_run`。统一接缝是 `ProcessRuntime`：`run(ProcessSpec)` 负责一次性命令，`prepare(ProcessSpec)` 只完成政策检查和 argv/env/cwd 包装，让 MCP SDK 继续负责协议流与关闭顺序。

`ProcessSpec` 永远使用显式 argv，runtime 永远 `shell=False`；`run_bash` 需要 shell 语法时，把选定的 bash/cmd/sh 本身作为 argv[0]。仓库内只有 `process.py` 可以直接调用 `subprocess`，从结构上防止新入口绕过 sandbox backend。

沙箱策略是 per-invocation 状态，模式为 `auto` / `required` / `off`，不写 session sidecar。session 可以跨机器和 workdir 恢复，而沙箱能力来自本次进程和当前平台；持久化旧探测结果反而可能形成错误授权。`FULL_ACCESS` 只跳过 executor 确认门，不改变沙箱策略。

`v0.7.0` 建立 backend protocol、能力报告和 `NoSandbox` 诚实降级，不虚构全平台硬墙：auto 缺后端时在启动环境与 trace 中明确说明，required 缺硬后端时在读取配置或创建会话前 fail-closed，off 表示用户显式关闭。首个硬后端选择 Bubblewrap：仅在 Linux 上通过真实 namespace probe 后启用；以只读系统运行目录、显式只读根、可写工作区、空 `/tmp`、独立 PID namespace 构造文件与进程边界，网络按 `inherit` / `deny` 策略决定是否共享。Linux CI 必须真实验证工作区外读写、宿主 loopback 和 PID namespace 逃逸，验证通过才报告 filesystem/network/process-tree capability。

MCP 不重新实现 transport。runtime 把包装后的 command/args/env/cwd 交给官方 SDK；SDK 仍负责 stdio JSON-RPC、优雅退出和进程树终止。MCP env 只包含 SDK 安全基础环境与 server 配置显式声明的键，避免把父进程中无关 API key/token 复制给每个 server。权限、输出截断和脱敏继续留在 executor，runtime 不建第二套工具管道。

## 决策 31：Hooks 按事件分组，Stop 是可否决的结束尝试

v0.8 第一版只读取 `<workdir>/.noval/hooks.json`，不引入用户级配置和跨来源合并。`hooks` 是以 `PreToolUse` / `PostToolUse` / `Stop` 为键的 object，各事件内数组顺序就是确定性的串行执行顺序；不再叠加 priority，也不并行运行共享同一工作区的验证命令。配置在用户回合边界刷新，工具链执行中途不热切换。

CommandHook 使用显式 argv 和 workdir，直接复用 `ProcessRuntime.run()`；它本身不再触发 Hooks，避免递归。每个命令都按 DANGEROUS 操作经过现有确认门，持久授权键由 Hook id 与规范化配置 hash 组成，因此同 id 的命令、参数或策略变化后必须重新批准。Hook 配置和命令输出都视为项目外部数据，不能覆盖 system、工具权限、path-jail、沙箱、脱敏或用户指令。

生命周期固定为：目标工具参数/权限校验 → PreToolUse → 工具执行 → PostToolUse → 模型候选最终回复 → Stop → completion judge。Pre 遇到首个 deny 短路且不执行目标工具；Post 已无法撤销工具，因此执行全部匹配项并把失败诊断附到对应 tool result；Stop 在候选回复展示前运行，deny/context 会把截断、脱敏后的框架反馈持久化并让模型继续修复。相同 Stop 失败后若没有新的工具活动，框架停止重复验证；达到 max_steps 时保留未通过状态，不调用 completion judge 宣称完成。

默认 `exit-code` 协议只把非零退出的 stdout/stderr 回传模型，成功输出不污染上下文；显式 `json` 协议严格限定为 `allow`、`deny(reason)`、`context(text)`。Hook 启动失败、timeout、非法协议输出都归一化为 deny，并有独立结构化 trace。Hooks 提供验证闭环，但不是安全边界的替代物。

## 决策 32：v0.9 用 canonical conversation model 取代 OpenAI wire 内核

`LLMClient` 虽然已经隔离 SDK，但 v0.8 及以前仍以 OpenAI-compatible message dict 作为 Agent、Session 与 Context 的共同语言，`tool_calls` / `tool_call_id` / `reasoning_content` 等 wire 字段会穿过核心层。v0.9 的目标不是多加一个 Provider 配置，而是把内核协议改成 Noval 自己的 canonical conversation model：消息由稳定 role 与有类型的 text / tool-call / tool-result block 组成，工具参数继续保留原始 JSON 字符串，由 executor 维持现有容错职责。

Provider 接缝改为 canonical request/response。`LLMResponse` 只返回一个 canonical assistant message、归一化 `TokenUsage`、`ProviderIdentity` 和安全框架元数据；不再同时暴露 `content` / `tool_calls` / `assistant_message` 三份可能漂移的表示，核心层也不再读取 SDK `raw` 对象。传给 Provider 的工具定义只包含 name / description / JSON schema，不携带 callable、risk 或 executor 状态。Provider SDK 异常统一归一化为带 kind、retryable、safe message 与 provider identity 的框架错误。

部分 Provider 在工具轮要求原样回放私有协议状态：DeepSeek 需要 `reasoning_content`，Anthropic thinking 模式需要未修改的 thinking / redacted-thinking blocks。它们不能被错误地提升成跨 Provider 语义，也不能简单丢弃。canonical assistant message 因此允许挂一个带 adapter id、schema version 与 opaque payload 的 adapter-owned replay state；核心只保存、计量大小并交还所属 adapter，不解析、不展示，也不送入 compactor、completion judge、运行日志或其它 Provider。切换 adapter 时只翻译 canonical 语义内容，外来 replay state 被忽略；任何无法表达的语义 block 必须显式失败，不能静默丢失。

Session 与 checkpoint 同步升级为 schema v2，只写 canonical 数据。项目尚无需要承担的历史包袱，因此不实现 v1 decoder、自动迁移、successor session 或混合格式追加；打开 v1 Session 时返回明确的 `UnsupportedSessionVersion`，旧文件保留但不读取、不改写、不删除，Session 列表应标记为不兼容。旧 checkpoint 同样不复用。v2 assistant 记录额外保存 provider / model / adapter schema provenance，为后续确定性回放建立来源，但 v0.9 不实现动态模型路由。

v0.9 用两个真实 wire adapter 证明边界：现有 OpenAI-compatible adapter 完成 canonical 双向翻译；Anthropic Messages adapter 作为可选依赖，只覆盖文本、system、用户定义 client tools、多工具调用、tool result/error、usage 与 thinking replay。server tools、streaming、多模态、prompt caching、异步 API、动态路由和自动 Provider fallback 均不进入本版本。旧配置缺少 provider 字段时继续默认 `openai-compatible`。
...
- [x] **统一子进程运行时（v0.7.0）**：shell、Skill、环境探测与 MCP 启动统一经过 `ProcessRuntime.run/prepare`；显式 argv、NoSandbox 状态、required fail-closed 和 MCP env 最小继承均有回归覆盖。
- [x] **Bubblewrap 硬沙箱（v0.7.0）**：Linux 实际可用性探测；工作区可写、显式根只读、宿主其它路径不可见；PID 与可选网络 namespace 隔离；专用 CI 执行真实逃逸测试。
- [x] **Hooks 与验证闭环（v0.8.0 MVP）**：项目级事件分组配置；组内确定性串行；CommandHook 复用权限与 ProcessRuntime；Pre 阻断、Post 诊断回流、Stop 修复再验证及重复失败保护均有离线回归。
