...
- **保护主干**：`main` 受 GitHub Ruleset 保护，禁止直接 push。所有变更必须通过短生命周期分支和 Pull Request 进入主干。
...
- **检查通过后合并**：Pull Request 必须通过必需的 `CI gate` 与 `Analyze Python`，并解决全部 review conversation。紧急情况下维护者仍只能在 Pull Request 中使用 bypass，且必须记录原因，不得绕过 PR 直接推送。
...
- **验证通过后合入主干**：功能分支推送并完成本地验证后，不应长期悬置。若代码范围清晰、验证通过且没有阻塞或待用户复核事项，应继续合并其 Pull Request 到 `main`；合入后同步关联 Issue。若只能部分完成或需要人工复核，必须明确说明原因并保持分支/Issue 状态可追踪。
...
noval/
  ...
  task.py       # 目标、行动收据、验证证据与独立 semantic judge
