...
`v0.7.0` 建立 backend protocol、能力报告和 `NoSandbox` 诚实降级，不虚构全平台硬墙：auto 缺后端时在启动环境与 trace 中明确说明，required 缺硬后端时在读取配置或创建会话前 fail-closed，off 表示用户显式关闭。`v0.7.x` 的首个硬后端选择 Bubblewrap：仅在 Linux 上通过真实 namespace probe 后启用；以只读系统运行目录、显式只读根、可写工作区、空 `/tmp`、独立 PID namespace 构造文件与进程边界，网络按 `inherit` / `deny` 策略决定是否共享。Linux CI 必须真实验证工作区外读写、宿主 loopback 和 PID namespace 逃逸，验证通过才报告 filesystem/network/process-tree capability。
...
- [ ] **Bubblewrap 硬沙箱（v0.7.x，待 Linux CI 首次验收）**：Linux 实际可用性探测；工作区可写、显式根只读、宿主其它路径不可见；PID 与可选网络 namespace 隔离；专用 CI 执行真实逃逸测试。
...
| `v0.7.0` | 统一子进程运行时与沙箱接缝 | `ProcessRuntime.run/prepare` 收束 shell、Skill、探测和 MCP；显式 argv；`NoSandbox` 诚实降级；`required` fail-closed | 只有 `process.py` 可直接调用 subprocess；MCP 保留官方 transport；timeout/trace 归一化且权限、截断、脱敏继续复用 executor 边界 |
| `v0.7.x` | Linux Bubblewrap 硬沙箱 | 可用性 probe；系统运行目录与显式 read roots 只读；workspace write root；空 `/tmp`；PID namespace；网络 `inherit` / `deny` | Linux CI 真实阻断工作区外读取、写入与宿主 loopback；PID namespace 可观测；不可用时 auto 诚实降级、required fail-closed |
| `v0.8.0` | Hooks 与验证闭环 | 生命周期事件 `PreToolUse` / `PostToolUse` / `Stop`；hook outcome 严格限定为 `allow` / `deny(reason)` / `context(text)`；CommandHook 复用 `ProcessRuntime.run`；PostToolUse 可跑 lint/test 并回喂诊断 | hook 不能覆盖 system、权限、脱敏、用户指令或 session 历史；失败隔离可观测；改后验证闭环可测 |
...
被版本门约束的长期能力：
- **长任务与记忆**：等 `v0.7.x` 至少一个平台硬后端与 `v0.8` 验证闭环稳定后推进；所有记忆必须有来源、时效、冲突与删除边界。
