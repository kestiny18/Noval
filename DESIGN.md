...
`v0.7.0` 建立 backend protocol、能力报告和 `NoSandbox` 诚实降级，不虚构全平台硬墙：auto 缺后端时在启动环境与 trace 中明确说明，required 缺硬后端时在读取配置或创建会话前 fail-closed，off 表示用户显式关闭。Bubblewrap 与其它平台硬后端属于 `v0.7.x`，必须有对应平台 CI 和真实逃逸测试后才能把 filesystem/network/process-tree capability 标为 true。
...
| `v0.7.0` | 统一子进程运行时与沙箱接缝 | `ProcessRuntime.run/prepare` 收束 shell、Skill、探测和 MCP；显式 argv；`NoSandbox` 诚实降级；`required` fail-closed；后续 `v0.7.x` 接 Bubblewrap / 平台 backend | 只有 `process.py` 可直接调用 subprocess；MCP 保留官方 transport；timeout/trace 归一化且权限、截断、脱敏继续复用 executor 边界 |
