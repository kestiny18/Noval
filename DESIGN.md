...
Noval 的 Skill 目标不是制定新规则，而是让 Agent 能发现并正确使用已有生态里的程序性知识。MVP 只兼容 Claude Code / Codex 常见的目录包形态：目录内有 `SKILL.md` 入口，frontmatter 提供 `name` / `description`，正文和附属文件按需读取。为避免把不同产品的隐式规则混成一锅，当前明确不扫描 Cursor 规则目录。

启动时扫描用户级和项目级 `.claude/skills`、`.codex/skills`、`.noval/skills`，把轻量索引追加到 system prompt。这个选择牺牲了一点 system 前缀稳定性，但换来模型在第一轮就知道“有哪些技能可用”；完整正文不进 system，只有模型决定使用某个 Skill 时才调用 `load_skill`。资源文件和脚本分别通过 `read_skill_resource`、`run_skill_script` 读取/执行，路径被限制在该 Skill 目录内。
...
- [x] **Skill 加载运行（MVP）**：复用 Claude Code / Codex 风格 `SKILL.md` 目录包；system 只注入轻量索引，完整正文、资源与脚本按需通过工具加载，脚本不绕过权限门。
