...
<p align="center">
  ...
  <sub>模型选择策略，Noval 让行动可信。</sub>
</p>

## 为什么还需要一个 Agent？

Agent 的“大脑”正在迅速变强，真正脆弱的部分却发生在模型之外：参数错了会不会崩、命令失控时谁来拦、输出太长时模型还能看见什么、进程中断后能否继续，以及模型说“完成了”时有没有外部证据。
...
许多框架通过固定 Planner、Executor、Reviewer 或工作流图来约束模型。Noval 选择更薄的 Harness：**模型负责理解目标和选择方法；Noval 负责现实、权限、连续性和证据。** 规划与复核仍然可用，但不再是每个任务必须经过的仪式。

完整思想见 [Noval 思想](PHILOSOPHY.zh-CN.md)。
...
实现约束见 [AGENTS.zh-CN.md](AGENTS.zh-CN.md)，当前规范架构见英文 [DESIGN.md](DESIGN.md)，v0.1-v0.10 的详细中文决策历史保存在 [DESIGN.zh-CN.md](DESIGN.zh-CN.md)。
...
- 贡献前请读 [CONTRIBUTING.zh-CN.md](CONTRIBUTING.zh-CN.md) 和 [AGENTS.zh-CN.md](AGENTS.zh-CN.md)。
- 公共契约、核心接缝或横切不变量变化需要新增或更新 [ADR](docs/adr/README.md)。
